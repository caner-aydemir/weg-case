...
const passwordContent = (
  <div className="flex justify-center text-amber-500 items-center  text-white  ">
    <Avatar
      size={"sm"}
...
function Login() {
  ...
  const usernameEventToggle = (event: any) => {
    setUsername(event.target.value); //Kullanıcıdan username datasını alıyorum
  };

  const passwordEventToggle = (event: any) => {
  ...
  const isDisabledPasswordInput =
    username.trim() !== "" && username.length > 0 ? false : true;
  return (
    <div
      className={
        "h-screen bg-gray-950 flex justify-center xs:p-2 items-center  text-white"
      }
    >
      <form onSubmit={handleSubmit} className="w-full flex justify-center">
        <div
          className={
            " gap-y-5 px-5 border-8 border-slate-800 bg-gray-900 rounded-2xl  py-5  " +
            "w-1/4 xs:w-full  h-auto flex justify-center items-center flex-col shadow-xl shadow-black"
          }
        >
          <div className={"flex items-center gap-x-3 "}>
            <p className="font-semibold xs:text-xl text-2xl text-center text-green-500  ">
              Wingie En Uygun <br /> Employee Panel
            </p>
          </div>
          <p className="font-bold text-lg xs:text-medium text-center">
          ...
          <p
            onClick={() => setOpenForgatPasswordModal(!openForgatPasswordModal)}
            className="hover:cursor-pointer w-full text-end text-xs underline"
          >
            Forgot password ?
          </p>
          <Button
      ...
      {openForgatPasswordModal && (
        <ForgatPasswordModal
          show={openForgatPasswordModal}
          close={() => setOpenForgatPasswordModal(!openForgatPasswordModal)}
        />
      )}
